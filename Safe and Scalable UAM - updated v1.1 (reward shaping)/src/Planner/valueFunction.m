...
if ~isempty(positivePeaks)
    ...
    discountFactorPositive = positivePeaks(:, DISC);
    ...
    [positiveValues,~] = valueOptimized(positiveDistance, rewardLimits, discountFactorPositive, rewards);
...
if ~isempty(negativePeaks)
    ...
    discountFactorNegative = negativePeaks(:, DISC)';
    ...
    [negativeValues, ~] = valueOptimized(negativeDistance, rewardLimits, discountFactorNegative, rewards);

    totalValues = positiveValues - negativeValues;

    % Reward shaping R = R+F
    totalValues = totalValues + ((positiveValues + (discountFactorPositive*positiveValues) ...
        - negativeValues + (0.97*negativeValues))  - ownship.bestVal(end,:));
